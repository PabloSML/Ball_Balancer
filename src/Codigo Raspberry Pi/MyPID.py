...
class PID:
    ...
    def calculate(self, error):
        ...
        #self.I = self.Ki*(error + self.prev_error)*self.dt/2 + self.I
        #self.D = 2*self.Kd*(error + self.prev_error)/(2*self.tau + self.dt) + (2*self.tau-self.dt)/(2*self.tau+self.dt)*self.D
